...
export async function listerCommandesEnRamassage(idHubAgent = null) {
  ...
  let requete = sb()
    .from("commandes")
    .select("id_commande, expediteur_nom, expediteur_tel, expediteur_adresse, code_ramassage, id_livreur_ramassage, cree_le, id_hub_prevu, hubs(nom)")
...
export async function listerColisDisponiblesPourLot(idHubAgent = null) {
  let requete = sb()
    .from("colis")
    .select("id_colis, destinataire_nom, code_zone, montant_livraison")
    .eq("statut", "AU_HUB")
...
export async function listerColisDuLot(idLot) {
  const { data, error } = await sb()
    .from("colis")
    .select("id_colis, destinataire_nom, code_zone, montant_livraison, statut")
    .eq("id_lot", idLot);
  if (error) throw error;
  return data || [];
}
...
export async function listerZones() {
  const { data, error } = await sb()
    ...
    .select("id, code_zone, description, actif, id_hub")
    .order("code_zone", { ascending: true });
  if (error) throw error;
  return data || [];
}

export async function enregistrerZone({ codeZone, description, idHub }, idEntreprise) {
  const { error } = await sb().from("zones_tarification").upsert(
    { id_entreprise: idEntreprise, code_zone: codeZone.toUpperCase(), description, id_hub: idHub || null, actif: true },
