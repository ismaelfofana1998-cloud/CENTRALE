...
import { afficherFlash, escapeHtml, tampon, ouvrirModale, fermerModale, copierTexte } from "../ui.js";
...
export async function monter(conteneur, actionsContainer, profil) {
  ...
  async function rafraichir() {
    ...
    if (!commandes.length) {
      conteneur.innerHTML = `<div class="etat-vide-tableau">Aucun ramassage en attente.</div>`;
      return;
    }
    conteneur.innerHTML = `
      ...
      <div class="bloc-tableau">
        <table class="donnees">
          <thead><tr><th><input type="checkbox" id="case-tout"></th><th>Commande</th><th>Expéditeur</th><th>Adresse</th><th>Hub</th><th>Livreur</th><th></th></tr></thead>
          <tbody>
            ${commandes.map((c) => `
              <tr>
                <td><input type="checkbox" class="case-commande" value="${c.id_commande}"></td>
                ...
                <td>${escapeHtml(c.expediteur_adresse || "—")}</td>
                <td>${escapeHtml(c.hubs?.nom || "—")}</td>
                <td>${c.id_livreur_ramassage
                  ? tampon("A_RAMASSER") + " " + (livreurs.find((l) => l.id_utilisateur === c.id_livreur_ramassage)?.nom || "")
                  : tampon("CREE")}</td>
    ...
    // est cochée ET qu'un livreur est choisi dans la liste du haut.
