...
export async function monter(conteneur, actionsContainer, profil) {
  actionsContainer.innerHTML = `
    <button class="btn btn-discret" id="btn-nouveau-tarif-paire">+ Tarif entre deux zones</button>
  ...
  async function rafraichir() {
    ...
    const nomZone = (code) => zones.find((z) => z.code_zone === code)?.description || code;
    ...
    conteneur.innerHTML = `
      <div class="bloc-tableau">
        ...
        ${zones.length ? `
          <table class="donnees">
            <thead><tr><th>Zone</th><th>Description</th><th>Hub de ramassage</th><th></th></tr></thead>
            <tbody>
              ${zones.map((z) => `
                <tr>
    ...
    actionsContainer.querySelector("#btn-nouveau-tarif-paire").onclick = () => ouvrirFormulairePaire(null, zones);
  }

  function ouvrirFormulairePaire(paire, zones) {
    const optionsZonesHtml = (valeurSelectionnee) => zones.map((z) =>
      `<option value="${z.code_zone}" ${z.code_zone === valeurSelectionnee ? "selected" : ""}>${escapeHtml(z.description || z.code_zone)}</option>`
    ).join("");
    ouvrirModale(`
  ...
  function ouvrirFormulaire(zone) {
    ouvrirModale(`
      ...
      <div class="formulaire">
        <div class="champ"><label>Code zone</label><input id="z-code" value="${zone?.code_zone || ""}" ${zone ? "disabled" : ""} placeholder="COCODY"></div>
        <div class="champ"><label>Description</label><input id="z-description" value="${zone?.description || ""}" placeholder="Cocody et environs"></div>
        <div class="champ">
    ...
    `, (boite) => {
      ...
      boite.querySelector("#btn-enregistrer").addEventListener("click", async (e) => {
        ...
        const r = await enregistrerZone({
          codeZone, description: boite.querySelector("#z-description").value.trim(),
