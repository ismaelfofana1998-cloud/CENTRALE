...
import { brancherAutocompletion, deviserZone } from "../geo.js";
...
function optionsZonesHtml(zones) {
  return `<option value="">Choisir…</option>${zones.map((z) => `<option value="${z.code_zone}">${escapeHtml(z.description || z.code_zone)}</option>`).join("")}`;
}

function ligneColisHtml(index, zones) {
  return `
    <div class="ligne-colis-form" data-ligne="${index}">
      ...
      <div class="champ">
        <label>Zone</label>
        <select class="dest-zone" required>${optionsZonesHtml(zones)}</select>
  ...
  async function ouvrirFormulaireCommande() {
    ...
    ouvrirModale(`
      ...
      <form id="form-commande" class="formulaire">
        ...
        <div class="champ"><label>Zone de ramassage</label><select id="exp-zone" required>${optionsZonesHtml(zones)}</select></div>
        <div class="champ">
        ...
        <div>
          ...
          <div class="liste-colis-form" id="liste-colis">${ligneColisHtml(0, zones)}</div>
    ...
    `, (boite) => {
      ...
      // logique, plus commentée, dans expediteur.js).
      function suggererZone(gps, selectEl) {
        ...
        if (zoneDevinee) {
          selectEl.value = zoneDevinee;
          selectEl.dataset.autoSuggere = "1";
      ...
      boite.querySelector("#btn-ajouter-ligne").addEventListener("click", () => {
        boite.querySelector("#liste-colis").insertAdjacentHTML("beforeend", ligneColisHtml(compteurLigne++, zones));
